...
if ($_GET['razdel']==3) {
    ...
    if($_GET['mf']) {
      $row = mysql_fetch_array(mysql_query("SELECT * FROM `inventory` WHERE `type` < 25 AND `type` != 12 AND `honor` = 0 AND `artefact` = 0 AND `podzem` != 1 AND `owner` = '{$_SESSION['uid']}' AND `id` = '{$_GET['mf']}' AND `name` NOT LIKE '% (мф)%' AND `name` NOT LIKE '%Букет%'  AND `name` NOT LIKE '%Мешок%' AND `setsale`=0 LIMIT 1;"));
      if ($row) {
        ...
        if($user['money'] < $row['cost'] OR !$row) {
            ...
            WHERE `id` = '{$_GET['mf']}' LIMIT 1;")) {
                ...
                mysql_query("INSERT INTO `delo` (`id` , `author` ,`pers`, `text`, `type`, `date`) VALUES ('','0','{$_SESSION['uid']}','Моцифицирована вещь \"".$row['name']."\" id:(cap".$row['id'].") [".$newduration."/".$row['maxdur']."] у \"".$user['login']."\" за ".round($row["cost"],2)." кр. ',1,'".time()."');");
    ...
    $data = mysql_query("SELECT * FROM `inventory` WHERE `type` < 25 AND `type` != 12 AND `artefact` = 0  AND `honor` = 0  AND `podzem` != 1 AND `owner` = '{$_SESSION['uid']}' AND `dressed` = 0 AND `name` NOT LIKE '% (мф)%' AND `name` NOT LIKE '%Букет%'  AND `name` NOT LIKE '%Мешок%' AND `setsale`=0 AND otdel != 50 ORDER by `update` DESC; ");
    while($row = mysql_fetch_array($data)) {
        $row['count'] = 1;
        if (!$row["cost"]) $row["cost"]=$row["ecost"]*11;
        if ($i==0) { $i = 1; $color = '#C7C7C7';} else { $i = 0; $color = '#D5D5D5'; }
