...
if ($_SERVER["REMOTE_ADDR"]=="127.0.0.1") define("DOCUMENTROOT","");
else define("DOCUMENTROOT","");
}
...
function geteffectval($val, $len, $maxlen) {
  ...
  function showbattlepers($id, $me, $battle, $mystrokes=0) {
    ...
    if ($me) {
      ...
      <tr>
      <td>".($p1?"<a href=\"fbattle.php?use=p1\" title=\"Использовать $p1[name]\"><img src=\"".IMGBASE."/i/sh/$p1[img]\"></a>":"<img alt=\"Пустой слот левый карман\" src=\"".IMGBASE."/i/w15.gif\">")."</td>
      <td><img alt=\"Пустой слот карман\" src=\"".IMGBASE."/i/w15.gif\"></td>
      <td>".($p2?"<a href=\"fbattle.php?use=p2\" title=\"Использовать $p2[name]\"><img src=\"".IMGBASE."/i/sh/$p2[img]\"></a>":"<img alt=\"Пустой слот правый карман\" src=\"".IMGBASE."/i/w15.gif\">")."</td>
      </tr>
      <tr>
...
//==============================================
        ...
        if(@$_POST['enemy'] && $this->enemy==$_POST['enemy'] && in_array($_POST['enemy'], $this->team_enemy)) {
          ...
          //if (!NOREFRESH) header ("Location:fbattle.php?fd=$_POST[enemy]");
        } else {
  ...
  function battle_end () {
    ...
    if($this->battle_data) {
      ...
      if(($t2life == 0 OR $t1life == 0) && ($charge[0] == 3 || $charge[0] == 9)) {
        ...
        if ($this->battle_data["type"]==UNLIMCHAOS || $issiege || $caverooms || $canalrooms)
        ...
        if ($this->battle_data["type"]!=4) {
          foreach ($this->t1 as $k => $v) {
            ...
            while ($rrow=mysql_fetch_row($us)) {
              if (($rrow[1]-$rrow[0])==1) $this->add_log('<span class=date>'.date("H:i").'</span> Внимание! У "'.$this->nick7($v).'" предмет "'.$rrow[2].'" в критическом состоянии! <BR><small>(на правах рекламы) <b>Ремонтная мастерская legendarybk</b>. Мы даем вторую жизнь старым вещам!</small><BR>');
              elseif (($rrow[1]-$rrow[0])==2) $this->add_log('<span class=date>'.date("H:i").'</span> Внимание! У "'.$this->nick7($v).'" предмет "'.$rrow[2].'" нуждается в ремонте! <BR><small>(на правах рекламы) <b>Ремонтная мастерская legendarybk</b>. Мы даем вторую жизнь старым вещам!</small><BR>');
            }
          }
          foreach ($this->t2 as $k => $v) {
            $us = mq('SELECT duration, maxdur, name FROM `inventory` WHERE `type` <> 25 AND `type` <> 187 AND `dressed` = 1 AND `owner` = \''.$v.'\';');
...
$result = (($baseexp[$def['level']])*($damage/$def['maxhp']))/9;			
} else {
...
<a href="javascript:void(0)" onClick="findlogin('Смена противника', 'fbattle.php?changeenemy=1', 'target', '')"><img src='<?=IMGBASE?>/i/ico_change.gif' width=16 height=19 style='cursor:pointer' <?=fastshow("Смена противника (".$fbattle->battleunits[$user["id"]]["changes"].")");?>></a>
<? } ?>
&nbsp;
...
?>
<div id=mes align=center style="padding-top:10px;padding-bottom:5px">
<?
//print_r($t1);
    $i=0;
...
$a = array(6,16);
echo "<img src='".IMGBASE."/img/battle/",$a[rand(1,5)],".jpg'>";
} elseif (@$fbattle->return > 1) {
echo "<img src='".IMGBASE."/img/battle/",rand(6,34),".jpg'>";
} elseif(@$exp[$user['id']] > 0) {
echo "<img src='".IMGBASE."/img/battle/",rand(35,50),".jpg'>";
} else {
echo "<img src='".IMGBASE."/img/battle/",rand(51,100),".jpg'>";
}
}
?>
</TD></TR>
...
<? if ($user["battle"]==0) { ?>
<div class="B1" style="text-align:center"><i>[ Все мертвы ]</i></div>
<? } ?>
<table width="100%">
<tr><td>
На данный момент вами нанесено урона: <B><?=(int)$fbattle->damage[$user['id']]?> HP</B>.
</td><td align="right">
<a href="logs.php?log=<?=$ll?>" target="_blank">Лог боя&raquo;&raquo;</a>
<br>(Бой идет с таймаутом <?=$fbattle->battle_data['timeout']?> мин.)</td></tr></table>
</FORM>
</div>
<?
...
if ($fbattle->needupdate) {
  $fbattle->update_battle();
  //if (!NOREFRESH) {
  //  echo "<script>document.location.replace('fbattle.php');</script>";
  //}
}
$fbattle->write_log();
