		...
		<table>
			...
			<td>
				<?
					...
					$data=mysql_query("SELECT `id`, `login`, `status`, `level`, `room`, `align`, (select `id` from `online` WHERE `date` >= ".(time()-60)." AND `id` = users.`id`) as `online` FROM `users` WHERE (`align` > 1 and `align` < 2) or `align`=3.999 order by align desc, login asc ;");
					while ($row = mysql_fetch_array($data)) {
						if ($row['online']<1) {
