  ...
  function getpersout($id, $me) {
    ...
    if($invis && !$me) {
      ...
      if ($user['helm'] > 0) {
        ...
        $ret.= '<img src="'.IMGBASE.'/i/w9.gif" width=60 height=60 alt="Пустой слот шлем" >';
      }
      $ret.= '</TD></TR><TR><TD style="BACKGROUND-IMAGE:none">';
      if ($user['naruchi'] > 0) {
        ...
        $ret.= '<img src="'.IMGBASE.'/i/w18.gif" width=60 height=40 alt="Пустой слот наручи" >';
      }
      $ret.= '</TD></TR><TR><TD style="BACKGROUND-IMAGE: none">';
      if ($user['weap'] > 0) {
        ...
        $ret.= '<img src="'.IMGBASE.'/i/w3.gif" width=60 height=60 alt="Пустой слот оружие" >';
      }
      $ret.= '</TD></TR><TR><TD style="BACKGROUND-IMAGE: none">';
      if ($user['bron'] > 0 || $user['rybax'] > 0 || $user['plaw'] > 0) {
        ...
        $ret.= '<img src="'.IMGBASE.'/i/w4.gif" width=60 height=80 alt="Пустой слот броня" >';
      }
      $ret.= '</TD></TR><TR><TD style="BACKGROUND-IMAGE: none">';
      if ($user['belt'] > 0) {
        ...
        $ret.= '<img src="'.IMGBASE.'/i/w5.gif" width=60 height=40 alt="Пустой слот пояс" >';
      }
    }
    ...
    if ($user['level'] > 3) $strtxt .= "Интеллект: ".$user['intel']."<BR>";
    if ($user['level'] > 6) $strtxt .= "Мудрость: ".$user['mudra']."<BR>";
    if ($user['level'] > 9) $strtxt .= "Духовность: ".$user['spirit']."<BR>";
    if ($user['level'] > 12) $strtxt .= "Воля: ".$user['will']."<BR>";
    if ($user['level'] > 15) $strtxt .= "Свобода духа: ".$user['freedom']."<BR>";
    if ($user['level'] > 18) $strtxt .= "Божественность: ".$user['god']."<BR>";
    ...
    if($me) {
      ...
      if($zver){
        ...
        <a href=\"zver_inv.php\">
        <IMG width=40 height=73 src='".IMGBASE."/i/shadow/$zver[shadow]' alt=\"$zver[login] [$zver[level]] (Перейтик настройкам)\">
        </a></div>";
      }
      $ret.= "<IMG border=0 src=\"".IMGBASE."/i/shadow/$user[sex]/$user[shadow]\" width=120 height=218 onmouseout='ghideshow();'  onmouseover='gfastshow(\"$strtxt\");'>";
    }
    ...
    if($invis && !$me) {
      ...
      if ($user['sergi'] > 0) {
        ...
        $ret.= '<img src="'.IMGBASE.'/i/w1.gif" width=60 height=20 alt="Пустой слот серьги" >';
      }
      $ret.= "</TD></TR><TR><TD style=\"BACKGROUND-IMAGE: none\">";
      if ($user['kulon'] > 0) {
        ...
        $ret.= '<img src="'.IMGBASE.'/i/w2.gif" width=60 height=20 alt="Пустой слот ожерелье" >';
      }
      ...
      if ($user['r1'] > 0) {
        ...
        $ret.= '<img src="'.IMGBASE.'/i/w6.gif" width=20 height=20 alt="Пустой слот кольцо" >';
      }
      $ret.= "</td><TD style=\"BACKGROUND-IMAGE: none\">";
      if ($user['r2'] > 0) {
      ...
      if ($user['perchi'] > 0) {
        ...
        $ret.= '<img src="'.IMGBASE.'/i/w11.gif" width=60 height=40 alt="Пустой слот перчатки" >';
      }
      $ret.= "</TD></TR><TR><TD style=\"BACKGROUND-IMAGE: none\">";
      if ($user['shit'] > 0) {
        ...
        $ret.= '<img src="'.IMGBASE.'/i/w10.gif" width=60 height=60 alt="Пустой слот щит" >';
      }
      $ret.= "</TD></TR><TR><TD style=\"BACKGROUND-IMAGE: none\">";
      if ($user['leg'] > 0) {
        ...
        $ret.= '<img src="'.IMGBASE.'/i/w19.gif" width=60 height=80 alt="Пустой слот поножи" >';
      }
      $ret.= "</TD></TR><TR><TD style=\"BACKGROUND-IMAGE: none\">";
      if ($user['boots'] > 0) {
        ...
        $ret.= '<img src="'.IMGBASE.'/i/w12.gif" width=60 height=40 alt="Пустой слот обувь" >';
      }
    }
    ...
    if ($me) {
      ...
      while($ch_eff = mysql_fetch_array($ch_eff1)){
        ...
        if($ch_eff['type']==395){$inf_el['img']='defender.gif'; $opp='награда'; $chas=60; $chastxt="час.";}elseif($ch_eff['type']==201){$inf_el['img']='spell_protect10.gif'; $opp='заклятие'; $chas=1; $chastxt="мин.";}elseif($ch_eff['type']==202){$inf_el['img']='spell_powerup10.gif'; $opp='заклятие'; $chas=1; $chastxt="мин.";}elseif($ch_eff['type']==1022){$inf_el['img']='hidden.gif'; $opp='заклятие'; $chas=1; $chastxt="мин.";}
          ...
          if ($ch_eff["sila"]>99 || $ch_eff["lovk"]>99 || $ch_eff["mudra"]>99 || $ch_eff["inta"]>99 || $ch_eff["intel"]>99) $opp='заклятие';
          $opp='эликсир'; $chas=1; $chastxt="мин.";
        }
        $strokes.= "<div style=\"position:absolute; left:{$left}px; top:{$top}px; width:120px; height:220px; z-index:2\"><IMG width=40 height=25 src='".IMGBASE."/i/misc/icon_$inf_el[img]' onmouseout='ghideshow();' onmouseover='gfastshow(\"<B>$ch_eff[name]</B> ($opp)<BR> еще ".ceil(($ch_eff['time']-time())/60/$chas)." $chastxt\")';> </div>";
      }
      ...
      while($ch_priem = mysql_fetch_array($ch_priem1)){
        ...
        $strokes.= "<div style=\"position:absolute; left:{$left}px; top:{$top}px; width:120px; height:220px; z-index:2\"><IMG width=40 height=25 src='".IMGBASE."/i/priem/$ch_priem[pr_name].gif' onmouseout='hideshow();' onmouseover='fastshow(\"<B>$inf_priem[name]</B> (приём)<BR><BR> $inf_priem[opisan]\")';> </div>";
      }
...
if (@$_REQUEST['special']) {
  if (@$strokes[$_REQUEST['special']]->target) {
    if ($_POST["main"]=="Невидимка") $_POST["main"]=$_SESSION["invis"];
