...
if ($cLottery) {
file_get_contents('http://lostcombats.com/lottery.php?cronstartlotery=whf784whfy7w8jfyw8hg745g3y75h7f23785yh38259648gjn6f6734h798h2q398fgsdhnit734');
$cLottery = mysql_result(mysql_query('SELECT COUNT(*) FROM lottery WHERE end = 0 AND date < NOW()'), 0, 0);
if (!$cLottery) {
sysmsg("Уважаемые игроки! Закончился очередной тираж Лотереи. Чтобы ознакомиться с результатами и получить выигрыш, посетите данную локацию в пригороде.");    
}
}
$tme1=getmicrotime();
...
$tr = mysql_fetch_array(mysql_query("SELECT * FROM `deztow_turnir` WHERE `active` = TRUE"));
    ...
    if(!$tr && $turnirstart[0] <= time() && $dd[0] >= 2 && $fromprev>300) {
      ...
      if (!$i) {
        ...
        $wmot=unserialize(implode("",file("/home/coomb141/domains/lostcombats.com/public_html/data/deztowshmot.dat")));
  ...
  if ($i<=time()) {  
    include DOCUMENTROOT."functions/movedtbots.php";
    ...
    if (!$dtf) {
      ...
      if (mysql_num_rows($r)>1) {
        ...
        include DOCUMENTROOT."/functions/makedtitems.php";
        mq("unlock tables");
      } else {
