    ...
    if ($can_use) {
        if ($select_target) {
            $ret.="
                ...
                " для приема <b nobr nowrap>$txt', 'fbattletest.php', 'target',  '$target_login', '".
                "$magic_type', '<INPUT type=hidden name=special value=$name>'".($free_cast?'':', 1').')">';
        } else {
            //s += free_cast ? '<A HREF="/fbattletest.php?special=' + name + '&r=' + rnd + '">': "<A style='cursor: pointer' onclick=\"b_confirm('fbattletest.php', '" + txt + "', '" + magic_type + "', '<INPUT type=hidden name=special value=" + name+ ">', 1)\">";
            $ret.="<A HREF=\"/fbattletest.php?special=$name&r=".time()."\">";
        }
    }
...
function geteffectval($val, $len, $maxlen) {
      ...
      foreach ($fbattle->battleunits[$user["id"]]["additdata"]["scrolls"] as $k=>$dress) {
        if ($dress) {
          if ($dress['magic'] && !$fbattle->battleunits[$user["id"]]["additdata"]["scrollused"] && !$dress["wait"]) {
            ...
            $ret.="<a  onclick=\"";
            if($magic['targeted']==1) {
              $ret.="okno('Введите название предмета', 'fbattletest.php?use=$dress[id]', 'target'); ";
            } else
            if($magic['targeted']==2) {
              $ret.="findlogin('Введите имя персонажа', 'fbattletest.php?use=$dress[id]', 'target', ''); ";
            } else
            if($magic['targeted']==4) {
              $ret.="note('Запрос', 'fbattletest.php?use=$dress[id]', 'target'); ";
            } else {
              $ret.="if(confirm('Использовать сейчас?')) { window.location='fbattletest.php?use=".$dress['id']."';}";
            }
            $ret.="\" href='#'>";
          }
  ...
  function showbattlepers($id, $me, $battle, $mystrokes=0) {
    ...
    if ($me) {
      ...
      <tr>
      <td>".($p1?"<a href=\"fbattletest.php?use=p1\" title=\"Использовать $p1[name]\"><img src=\"".IMGBASE."/i/sh/$p1[img]\"></a>":"<img alt=\"Пустой слот левый карман\" src=\"".IMGBASE."/i/w15.gif\">")."</td>
      <td><img alt=\"Пустой слот карман\" src=\"".IMGBASE."/i/w15.gif\"></td>
      <td>".($p2?"<a href=\"fbattletest.php?use=p2\" title=\"Использовать $p2[name]\"><img src=\"".IMGBASE."/i/sh/$p2[img]\"></a>":"<img alt=\"Пустой слот правый карман\" src=\"".IMGBASE."/i/w15.gif\">")."</td>
      </tr>
      <tr>
...
//==============================================
        ...
        if(@$_POST['enemy'] && $this->enemy==$_POST['enemy'] && in_array($_POST['enemy'], $this->team_enemy)) {
          ...
          //if (!NOREFRESH) header ("Location:fbattletest.php?fd=$_POST[enemy]");
        } else {
      ...
      if(($t2life == 0 OR $t1life == 0) && ($charge[0] == 3 || $charge[0] == 9)) {
        ...
        if(in_array($ss[0],$this->t1)) {
          ...
          if (in_array($user["room"], $caverooms)) {
            ...
            //leader='$user[caveleader]' and x='$x' and y='$y'");
        ...
        unset($this->battle);
        //header("Location: fbattletest.php");  die();
        return true;
        // =================================================================
      }
    }
  ...
  function end_gora() {
      // я - царь горы
          if ($this->get_timeout()) {
              ...
              //header("Location: fbattletest.php?fd=all&batl=".$this->user['battle']);
          }
  }
    ...
    if ($enable) {
      ...
      if ($ok) {
        ...
        if (@$strokes[$priem]->refresh) {
          //if (!NOREFRESH) header ("Location:fbattletest.php?fd=".$fbattle->enemy);
          $fbattle->needupdate=1;
          //die;
        }
        if(@$_POST['enemy']>0 && $emptyhit) {
          ...
          //if (!NOREFRESH) header ("Location:fbattletest.php");
          //die;
        }
        $fbattle->write_log();
...
if ($fbattle->needupdate) {
  ...
  if (@$fbattle->needrefresh || $pershp<=0) {
    ...
    header ("Location:fbattletest.php?".time()."&if=".@$_GET["if"]);
    $fbattle->updateaddit();
    die;
  }
} else {
...
<div id="screen">
<form name="emptyhit" action="fbattletest.php" method="post" style="margin:0px;padding:0px;visibility:hidden;display:none">
...
<a href="javascript:void(0)" onClick="findlogin('Смена противника', 'fbattletest.php?changeenemy=1', 'target', '')"><img src='<?=IMGBASE?>/i/ico_change.gif' width=16 height=19 style='cursor:pointer' <?=fastshow("Смена противника (".$fbattle->battleunits[$user["id"]]["changes"].")");?>></a>
<? } ?>
&nbsp;
...
<td align=center>
&nbsp;
<INPUT TYPE=button value="Обновить" onClick="this.disabled = true; document.location.href='fbattletest.php';">
</td>
            ...
            elseif($user['hp'] <= 0 && $fbattle->battle) {
                if ($fbattle->battle_data["quest"]==2) {
                  ...
                  if ($timeloser) echo "<center><BR>Персонаж ".mqfa1("select login from users where id='$timeloser'")." долго не делает свой ход, вы можете исключить его из боя.<BR>
                    <INPUT TYPE=button value=\"Признать его проигравшим\" onclick=\"document.location.href='fbattletest.php?killplayer=$timeloser';\"><BR>
...
//print_r($t1);
    ...
    /*if (strpos($ffs,"[-") || strpos($ffs,"[0")) {
      $fbattle->fast_death();
      header("location:fbattletest.php");
    }*/
...
if ($fbattle->needupdate) {
  $fbattle->update_battle();
  //if (!NOREFRESH) {
  //  echo "<script>document.location.replace('fbattletest.php');</script>";
  //}
}
$fbattle->write_log();
