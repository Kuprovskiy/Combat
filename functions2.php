...
function getuserdata($id=0) {
  ...
  if (!@$_SESSION["allchecked"]) {
    ...
    if (mysql_num_rows($r)==0) {
      mq("insert into allusers (select * from users where id='{$id}')");
    }
  }
  ...
  if ($rec["hp"]<$rec["maxhp"] && time()>$rec["fullhptime"] && !$rec["battle"] && $rec["in_tower"]<>62 && ($rec["hp"]>0 || canregendead($rec))) {
    if (haseffect($rec["data"], 28)) {
      $fulltime=300;
    } elseif (haseffect($rec["data"], HPADDICTIONEFFECT)) {
      getadditdata($rec);
      $fulltime=30*addictval($rec["hpaddict"])+600;
    } else $fulltime=600;
    $delta=ceil((time()-$rec["fullhptime"])/$fulltime*$rec["maxhp"]);
    $rec["hp"]+=$delta;
    if ($rec["hp"]>=$rec["maxhp"]) $rec["hp"]=$rec["maxhp"]-1;
  }  
  return $rec;
}
...
//if ((int)$user['id']!=111 && strpos($_SERVER["PHP_SELF"],"ch.php")!==false) 
if ($user) mq("UPDATE `online` SET `real_time` = ".time()." WHERE `id` = {$user['id']};");
...
"12500" => array (3,1,1,80,1,14000,0),
...
"30000" => array (5,1,1,90,1,60000,0),
...
"120000000" => array (9,1,20,2000,1,130000000,6),
  ...
  if ($user["nextup"] && $user['exp'] >= $user['nextup'] && !$user['in_tower'] && $user["id"]) {
    ...
    if($exptable[$user['nextup']][4]) {
      ...
      if(!empty($user["refer"]) && $user["level"]>0) {
        //$lgns=mysql_fetch_array(mq("SELECT `login` FROM `users` WHERE `id`='$data[refer]'"));
        if ($user["refer"]==7918 || $user["refer"]==4217) {
          mq("UPDATE `users` SET `ekr`= `ekr` + ".($user["level"]*2)." WHERE `id`='$user[refer]'");
          mq("UPDATE `allusers` SET `ekr`=`ekr` + ".($user["level"]*2)." WHERE `id`='$user[refer]'");
        } else {
          mq("UPDATE `users` SET `money`= `money` + ".($user["level"]*2)." WHERE `id`='$user[refer]'");
          mq("UPDATE `allusers` SET `money`=`money` + ".($user["level"]*2)." WHERE `id`='$user[refer]'");
        }


        $us = mysql_fetch_array(mq("select `id` from `online` WHERE `date` >= ".(time()-60)." AND `id` = '{$user['refer']}' LIMIT 1;"));
        if($us[0]){
          addchp ('<font color=red>Внимание!</font> <font color=\"Black\">Персонаж <B>'.$user['login'].'</B> перешел на '.($user['level']+1).' уровень. Вам перечислено '.($user["level"]*2).'  '.($user["refer"]==7918 || $user["refer"]==4217?"е":"").'кр.</font>   ','{[]}'.nick7 ($user['refer']).'{[]}');
        } else {
          mq("INSERT INTO `telegraph` (`owner`,`date`,`text`) values ('".$user['refer']."','','".'<font color=red>Внимание!</font> <font color=\"Black\">Персонаж <B>'.$user['login'].'</B> перешел на '.($user['level']+1).' уровень. Вам перечислено '.($user['level']*2).'  '.($user["refer"]==7918 || $user["refer"]==4217?"е":"").'кр.</font> '."');");
        }
      }
    }
  ...
  //if ($user["id"]==7) $nodrink=array();
  $noattack=array();
  if ($user["id"]==3671) $noattackrooms=array();
  else $noattackrooms=array(WARROOM, 31, 35, 101, 102, 103, 104, 105, 402, 403, 404, 43, 45, 47, 48, 51, 52, 54, 56, 57, 62, 63, 301, 302,
  ...
  "Готический замок - мастерские","Готический замок - комнаты отдыха","Лотерея Сталкеров","Комната Знахаря",
  ...
  54=>"Лаборатория Алхимика", 56=>"Вход в Пещеру кристаллов", 57=>"Пещера Кристаллов", 59=>"Книжный магазин",
  ...
  70=>"Подгорье", 71=>"Вход в башню", 72=>"Башня цвергов", 
  80=>"Магический круг",
...
718=>"Обитель монахов", 719=>"Домик мага", 720=>"Харчевня", 721=>"Цветник",
);

  $canalrooms=array(403, 48, 52);
  $canalenters=array(402, 47, 51, 60, 64, 71, 301);
  $caverooms=array(302, 61, 65, 72);
  $battlefields=array(57, 63);
  $battleenters=array(56, 62);

  header("Cache-Control: no-cache");

function getvar($v) { return mqfa1("select value from variables where var='$v'"); }

function setvar($var, $val) { mq("update variables set valur='$val' where var='$var'"); }
...
//      mq("DELETE FROM `effects` WHERE `time` <= ".time()." AND `owner` = '".$id."';");

    $owntravmadb=mq("SELECT type, id, sila, lovk, inta, intel, mudra, gmana, owner, name, isp, mfdhit, mfdmag, mfdkol, mfdrej, mfdrub, mfddrob, mfdfire, mfdair, mfdwater, mfdearth, time FROM effects WHERE (`time` <= ".time()."  or (`time` <= ".time()."+600 and isp=0) ) AND `owner` = ".$id.";");
    $he=0;
    $canremove=1;
    ...
    ///путы от магии свободы
      ...
      /////
        ...
        if ($owntravma['type']==49 || $owntravma["type"]==26 || $owntravma["gmana"]) {
          resetmax($user1["id"]);
        }       
    ...
    if (($user1["hp"]>0 || !$user1["in_tower"]) && $user1["hp"]<$user1["maxhp"] && $user["in_tower"]<>62) {
      ...
      if (!@$noheal) { 
        ...
        } elseif (haseffect($user1["data"], HPADDICTIONEFFECT)) {
          getadditdata($user1);
        ...
        if($user1['maxhp']<=100){
          if ((time()-$user1['fullhptime'])/60 >= 1) {
            ...
            /*$user169 = mysql_fetch_array(mq("SELECT hp,maxhp, level, spirit FROM `users` WHERE `id` = '".$id."' LIMIT 1;"));
            if ($user169["level"]>=10) $md=40;
            elseif ($user169["level"]>=9) $md=30;
            elseif ($user169["level"]>=8) $md=20;
            else $md=10;
            $md+=$user169["spirit"];
            $hz = floor(($user169['hp']/($user169['maxhp']/100))*$md);
            mq("UPDATE `users` SET `s_duh` = '".$hz."' WHERE `id` = '".$id."' LIMIT 1;");*/
          }
        } else {
          mq("UPDATE `users` SET `hp` = if(`hp`+(((".time()."-`fullhptime`)/60)*(`maxhp`/10)*$mult)<maxhp, `hp`+(((".time()."-`fullhptime`)/60)*(`maxhp`/10)*$mult),maxhp), `fullhptime` = ".time()." WHERE  `hp` < `maxhp` AND `id` = '".$id."'");
          /*$user169 = mysql_fetch_array(mq("SELECT hp,maxhp, level, spirit FROM `users` WHERE `id` = '".$id."' LIMIT 1;"));
          ...
          $md+=$user169["spirit"];
          $hz = round(($user169['hp']/($user169['maxhp']/100))*$md);
          mq("UPDATE `users` SET `s_duh`='".$hz."' WHERE `id` = '".$id."' LIMIT 1;");*/
        }
      }
    }
    ...
    if ($user1["mana"]<$user1["maxmana"]) {
      ...
      if ($he!==false) {
        $manamult*=0.04*$user1["data"]["effects"][$he]["addict"]+2;
      } elseif (haseffect($user1["data"], MANAADDICTIONEFFECT)) {
        getadditdata($user1);
...
function fullnick($user, $showinvis=0, $class=0) {
  if (!is_array($user)) {
    $id=$user;
    $user=mqfa("select id, align, klan, login, level, invis from users where id='$id'");
    if (!$user) $user=mqfa("select id, align, klan, login, level, invis from allusers where id='$id'");
  }
  if ($showinvis && $user["invis"]) {
    $user["level"]="??";$user["login"]="невидимка";
    $user["align"]=0;$user["klan"]="";
  }
  return ($class?"<span class=\"$class\">":"")."<img src=\"".IMGBASE."/i/align_".($user['align']>0?$user['align']:"0").".gif\"> ".($user['klan']?"<img title=\"$user[klan]\" src=\"".IMGBASE."/i/klan/$user[klan].gif\">":"")." <B>$user[login]</B> [$user[level]] ".($user["login"]=="невидимка"?"":"<a href=inf.php?$user[id] target=_blank><IMG SRC=".IMGBASE."/i/inf.gif WIDTH=12 HEIGHT=11 ALT=\"Инф. о $user[login]\"></a>").($class?"</span>":"");
}
...
function showpersout($id,$pas = 0,$battle = 0,$me = 0,$show_pr = 0) {
  ...
  </TD><TD><TABLE border=0 cellSpacing=0 cellPadding=0 width=\"100%\">
  ...
  $zver=mysql_fetch_array(mq("SELECT shadow,login,level FROM `users` WHERE `id` = '".$user1['zver_id']."' LIMIT 1;"));
  ...
  if(!$pas && !$battle){
    ...
    <TR><TD></TD>";
    $data = mysql_fetch_array(mq("select * from `online` WHERE `date` >= ".(time()-60)." AND `id` = ".$user1['id'].";"));
/*      $dd = mq("SELECT * FROM `effects` WHERE `owner` = ".$user1['id'].";");
    ...
    if (!$battle) {
      if ($pas){ 
        $ret.="<TR><TD align=middle colSpan=2><B>Virt<img src=\"".IMGBASE."/i/misc/forum/fo1.gif\">City</B></TD></TR>
        <TR><TD colSpan=2>
        <SMALL>";
        ...
        $invis = mqfa1("select `time` from effects where owner='$user1[id]' and type='1022'");
        if ($invis>time()) {
          $data['id']=null;
        ...
        if ($data['id'] != null or ($user1['id'] == 99 && vrag=="on")) {
          ...
          $ret.='<center>Персонаж сейчас находится в клубе.</center><center>Локация : <B>"'.$rrm.'"</B></center>';
        } else {
          if (!$user1["bot"]) {
...
/*          if ($trt) {
      ...
      if (!$pas) {
        ...
        Благородство: <b>$user1[honorpoints]</b>&nbsp;<img width=12 height=10 alt=\"Благородство:\" src=".IMGBASE."/i/fixed_on.gif><BR>";
      }
      if ($user1['level'] < 2 && !$pas) {
        //$ret.="Зубов:&nbsp;<SMALL><IMG SRC=\"".IMGBASE."/i/misc/zub_low4.gif\" ALT=\"Гнилой Зуб\" >0.00-<font color=red>нету</font></SMALL><BR>";
      }
      if ($user1['level'] > 1 && !$pas) {
        $ret.="Кристаллы:&nbsp;<SMALL><IMG width=14 height=14 SRC=\"".IMGBASE."/i/rkrist.gif\" ALT=\"Кристаллы\" ></SMALL><BR>";
      }
      if($user1['klan'] && !$pas) {
...
function canhold($user, $row) {
  ...
  ($row['type'] < 26 || $row['type']==50 || $row['type']==187)&&
  ($row['needident'] == 0)) return true;
  return false;
}

function showitem ($row, $link="") {
    if(($row['type']>=25 && $row['maxdur'] <= $row['duration']) || (($row['dategoden'] && $row['dategoden'] <= time()))) {
      if (!$row["gift"] && strpos($row["name"], "Роба Блаженного Жреца")===false) destructitem($row['id']);
    }
    ...
    // end if shop
if ($row['destinyinv']>0) {
        echo "<a href=#>{$row['name']}</a><img src=".IMGBASE."/i/align_{$row['nalign']}.gif> (Масса: {$row['massa']})<img src=\"".IMGBASE."/i/klan/{$row['clan']}.gif\"><img src=".IMGBASE."/i/destiny{$row['destinyinv']}.gif alt=\"Этот предмет связан с Вами общей судьбой. Вы не можете передать его кому-либо еще.\"><img src=".IMGBASE."/i/artefact{$row['artefact']}.gif>".(($row['present'])?' <IMG SRC="'.IMGBASE.'/i/podarok.gif" WIDTH="16" HEIGHT="18" BORDER=0 TITLE="Этот предмет вам подарил '.$row['present'].'. Вы не сможете передать этот предмет кому-либо еще." ALT="Этот предмет вам подарил '.$row['present'].'. Вы не сможете передать этот предмет кому-либо еще.">':"")."<BR>";
    }
elseif ($row['destiny']>0) {
        echo "<a href=#>{$row['name']}</a><img src=".IMGBASE."/i/align_{$row['nalign']}.gif> (Масса: {$row['massa']})<img src=\"".IMGBASE."/i/klan/{$row['clan']}.gif\"><img src=".IMGBASE."/i/destiny{$row['destiny']}.gif alt=\"Этот предмет будет связан с Вами общей судьбой. Вы не сможете передать его кому-либо еще.\"><img src=".IMGBASE."/i/artefact{$row['artefact']}.gif>".(($row['present'])?' <IMG SRC="'.IMGBASE.'/i/podarok.gif" WIDTH="16" HEIGHT="18" BORDER=0 TITLE="Этот предмет вам подарил '.$row['present'].'. Вы не сможете передать этот предмет кому-либо еще." ALT="Этот предмет вам подарил '.$row['present'].'. Вы не сможете передать этот предмет кому-либо еще.">':"")."<BR>";
    }else{
    echo "<a href=#>{$row['name']}</a>";if($row['koll']>'0'){echo " <b>(x{$row['koll']})</b>";}print"<img src=\"".IMGBASE."/i/align_{$row['nalign']}.gif\"> (Масса: {$row['massa']})<img src=\"".IMGBASE."/i/klan/{$row['clan']}.gif\"><img src=".IMGBASE."/i/destiny{$row['destiny']}.gif><img src=".IMGBASE."/i/artefact{$row['artefact']}.gif>".(($row['present'])?' <IMG SRC="'.IMGBASE.'/i/podarok.gif" WIDTH="16" HEIGHT="18" BORDER=0 TITLE="Этот предмет вам подарил '.$row['present'].'. Вы не сможете передать этот предмет кому-либо еще." ALT="Этот предмет вам подарил '.$row['present'].'. Вы не сможете передать этот предмет кому-либо еще.">':"")."<BR>";
}

    if($row['ecost']>0) { echo "<b>Цена: {$row['ecost']} екр.".($_SESSION["uid"]==7?"/$row[cost]":"")."</b> &nbsp; &nbsp;"; } elseif($row['cost']>0 && $row['silverzeton']==0 && $row['goldzeton']==0 && $row['zeton']==0 && $row["honor_cost"]==0) { echo "<b>Цена: {$row['cost']} кр.</b> &nbsp; &nbsp;"; }
    if($row['honor_cost']>0) { echo "<b>Цена: {$row['honor_cost']} благородства.</b> &nbsp; &nbsp;"; }
if($row['zeton']>0) { echo "<br><b>Жетон: {$row['zeton']}</b>&nbsp<br>"; }
if($row['silverzeton']>0) { echo "<br><b>Серебряный жетон: {$row['silverzeton']}</b>&nbsp<br>"; }
if($row['goldzeton']>0) { echo "<br><b>Золотой жетон: {$row['goldzeton']}</b>&nbsp<br>"; }
        if(@$row['count']) {
            echo "<small>(количество: {$row['count']})</small>";
        }
    if ($row["type"]==3) {
      if ($row["otdel"]==1) echo "<br>Тип оружия: нож";
      if ($row["otdel"]==11) echo "<br>Тип оружия: топор";
      if ($row["otdel"]==12) echo "<br>Тип оружия: дубина";
      if ($row["otdel"]==13) echo "<br>Тип оружия: меч";
      if ($row["otdel"]==30) echo "<br>Тип оружия: посох";
    }
    echo "<br>";
if($row['type']!=200){
...
if (!$row['needident']) {
        ...
        if ($row["dategoden"]>time()) echo (($row['goden'])?"Срок годности: {$row['goden']} дн. ".((!$row['count'])?"(до ".date("Y.m.d H:i",$row['dategoden']).")":"")."<BR>":"");
        elseif ($row["goden"]) echo "Срок годности: {$row['goden']} дн.<BR>";
        echo itemreqs($row);
        echo itemmfs($row);
        if ($row["dategoden"] && $row["dategoden"]<time() && $row["owner"]<_BOTSEPARATOR_) echo "<small><font color=maroon>Предмет устарел</font></small><BR>";
        }
        else { echo "<font color=maroon><B>Свойства предмета не идентифицированы</B></font><BR>"; }
        //echo "<small>№ $row[id]</small>
        //<!-- <small>Сделано в Virt City</small>-->";
        echo "</td></TR>";
}
...
function itemreqs($row, $user1=0) {
  global $user;
  if (!$user1) $user1=&$user;
  return (($row['nsila'] OR $row['nlovk'] OR $row['ninta'] OR $row['nvinos'] OR $row['nlevel'] OR $row['nintel'] OR $row['nmudra'] OR $row['nnoj'] OR $row['ntopor'] OR $row['ndubina'] OR $row['nmech'] OR $row['nfire'] OR $row['nwater'] OR $row['nair'] OR $row['nearth'] OR $row['nearth'] OR $row['nlight'] OR $row['ngray'] OR $row['ndark'])?"<b>Требуется минимальное:</b><BR>":"")."
...
function itemmfs($row) {
  ...
  if ($row['letter']) $ret["addit"].="Количество символов: ".strlen($row['letter'])."</div>";
  ...
  if ($row['podzem']==1) $ret["addit"].="<br><font style='font-size:11px; color:#990000'>Предмет из подземелья</font><BR>";
...
function showeffects($user) {
  $r=mq('SELECT * FROM `effects` WHERE `owner` ='.$user.' and (type=11 or type=12 or type=13 or type=14 or type=22 or type=26 or type=31 or type=32 or type=33 or type=34 or type=35 or type=38 or type=49 or type=185 or type=186 or `type`=187 or `type`=188 or `type`=395 or `type`=396 or `type`=201 or `type`=202 or `type`=1022 or `type`=54 or type=28 or type=29 or type=30)');
  $i=0;
  $ret="";
  $effectdata["Хлеб с мясом"]=array("img"=>"icon_food.jpg", "type"=>"еда");
  $effectdata["Бутерброд с мясом"]=array("img"=>"icon_food.jpg", "type"=>"еда");
  $effectdata["Окорочок"]=array("img"=>"icon_food.jpg", "type"=>"еда");
  $effectdata["Бутерброд -Завтрак рыцаря-"]=array("img"=>"icon_food.jpg", "type"=>"еда");
  $effectdata["Жажда жизни +1"]=array("img"=>"lt1.gif", "type"=>"заклятие");
  $effectdata["Жажда жизни +2"]=array("img"=>"lt2.gif", "type"=>"заклятие");
  $effectdata["Жажда жизни +3"]=array("img"=>"lt3.gif", "type"=>"заклятие");
  $effectdata["Жажда жизни +4"]=array("img"=>"lt4.gif", "type"=>"заклятие");
  $effectdata["Жажда жизни +5"]=array("img"=>"lt5.gif", "type"=>"заклятие");
  $effectdata["Жажда жизни +6"]=array("img"=>"lt6.gif", "type"=>"заклятие");
  $effectdata["Иссушение -1"]=array("img"=>"mlt1.gif", "type"=>"заклятие");
  $effectdata["Иссушение -2"]=array("img"=>"mlt2.gif", "type"=>"заклятие");
  $effectdata["Иссушение -3"]=array("img"=>"mlt3.gif", "type"=>"заклятие");
  $effectdata["Иссушение -4"]=array("img"=>"mlt4.gif", "type"=>"заклятие");
  $effectdata["Иссушение -5"]=array("img"=>"mlt5.gif", "type"=>"заклятие");
  $effectdata["Эликсир восстановления"]=array("img"=>"regeneration.gif", "type"=>"эликсир");
  $effectdata["Эликсир потока"]=array("img"=>"manaregen.gif", "type"=>"эликсир");
  $effectdata["Кредит"]=array("img"=>"credit.gif", "type"=>"эффект");
  $effectdata["Просроченный кредит"]=array("img"=>"unpaidcredit.gif", "type"=>"эффект");
  $effectdata["Защита от травм"]=array("img"=>"icon_travmaresist.gif", "type"=>"эффект");
  $effectdata["Победитель общего побоища"]=array("img"=>"icon_winner.gif", "type"=>"эффект");
  while($rec=mysql_fetch_array($r)) {
    $i++;
    list($left, $top)=effectpos($i);
    if ($rec["type"]==ADDICTIONEFFECT) {
      if ($rec["mfdhit"]) $img="mfdhit";if ($rec["mfdkol"]) $img="mfdkol";if ($rec["mfdrub"]) $img="mfdrub";if ($rec["mfdrej"]) $img="mfdrej";if ($rec["mfddrob"]) $img="mfddrob";
      if ($rec["mfdmag"]) $img="mfdmag";if ($rec["mfdfire"]) $img="mfdfire";if ($rec["mfdwater"]) $img="mfdwater";if ($rec["mfdair"]) $img="mfdair";if ($rec["mfdearth"]) $img="mfdearth";
      if ($rec["sila"]) $img="sila";if ($rec["lovk"]) $img="lovk";if ($rec["inta"]) $img="inta";if ($rec["intel"]) $img="intel";
      
      $chas=1;
      $ret.="<div style=\"position:absolute; left:{$left}px; top:{$top}px; width:40px; height:25px; z-index:2\">
      <IMG width=40 height=25 src='".IMGBASE."/i/misc/icon_$img.gif' onmouseout='ghideshow();' onmouseover='gfastshow(\"<B>$rec[name]</B> (эффект)<BR> еще ".lefttime($rec["time"])."\")';> </div>";      
    } elseif ($rec["type"]==HPADDICTIONEFFECT) {
      $chas=1;
      $ret.="<div style=\"position:absolute; left:{$left}px; top:{$top}px; width:40px; height:25px; z-index:2\">
      <IMG width=40 height=25 src='".IMGBASE."/i/misc/icon_hpaddict.gif' onmouseout='ghideshow();' onmouseover='gfastshow(\"<B>$rec[name]</B> (эффект)<BR> еще ".lefttime($rec["time"])."\")';> </div>";      
    } elseif ($rec["type"]==MANAADDICTIONEFFECT) {
      $chas=1;
      $ret.="<div style=\"position:absolute; left:{$left}px; top:{$top}px; width:40px; height:25px; z-index:2\">
      <IMG width=40 height=25 src='".IMGBASE."/i/misc/icon_manaaddict.gif' onmouseout='ghideshow();' onmouseover='gfastshow(\"<B>$rec[name]</B> (эффект)<BR> еще ".lefttime($rec["time"])."\")';> </div>";      
    } elseif ($effectdata[$rec["name"]]) {
      $chas=1;
      $ret.="<div style=\"position:absolute; left:{$left}px; top:{$top}px; width:40px; height:25px; z-index:2\">
      <IMG width=40 height=25 src='".IMGBASE."/i/misc/".$effectdata[$rec["name"]]["img"]."' onmouseout='ghideshow();' onmouseover='gfastshow(\"<B>$rec[name]</B> (".$effectdata[$rec["name"]]["type"].")<BR> еще ".lefttime($rec["time"])."\")';> </div>";
    } elseif ($rec["type"]==11 || $rec["type"]==12 || $rec["type"]==13 || $rec["type"]==14) {
      $chas=1;
      $ret.="<div style=\"position:absolute; left:{$left}px; top:{$top}px; width:40px; height:25px; z-index:2\">
      <IMG width=40 height=25 src='".IMGBASE."/i/magic/travma.gif' onmouseout='ghideshow();' onmouseover='gfastshow(\"<B>$rec[name]</B> (травма)<BR> еще ".lefttime($rec["time"])."\")';> </div>";
    } elseif ($rec["type"]==30) {
      $chas=1;
      $ret.="<div style=\"position:absolute; left:{$left}px; top:{$top}px; width:40px; height:25px; z-index:2\">
      <IMG width=40 height=25 src='".IMGBASE."/i/misc/pregnant.gif' onmouseout='ghideshow();' onmouseover='gfastshow(\"<B>$rec[name]</B> (эффект)<BR> еще ".lefttime($rec["time"])."\")';> </div>";
    } elseif ($rec["type"]==38) {
      $ret.="<div style=\"position:absolute; left:{$left}px; top:{$top}px; width:40px; height:25px; z-index:2\">
      <IMG width=40 height=25 src='".IMGBASE."/i/magic/".$rec["type"].".gif' onmouseout='ghideshow();' onmouseover='gfastshow(\"<B>$rec[name]</B> (эффект)<BR> еще ".lefttime($rec["time"])."\")';> </div>";
    } else {
      ...
      if ($rec["type"]>395 && $rec["type"]<1000) {
        $inf_el['img']="prize$rec[type].gif"; $opp='награда';
      } elseif($rec['type']==395){$inf_el['img']='defender.gif'; $opp='награда'; }elseif($rec['type']==201){$inf_el['img']='spell_protect10.gif'; $opp='заклятие'; }elseif($rec['type']==202){$inf_el['img']='spell_powerup10.gif'; $opp='заклятие'; }elseif($rec['type']==1022){$inf_el['img']='hidden.gif'; $opp='заклятие';}
      elseif ($rec["type"]==187) {$opp='заклятие';}
      elseif ($rec["type"]==54) {$rec['name']="Изучение: $rec[name]".(strpos($rec["name"], "Знание")?"":" (приём)");$opp='эффект';}
      else{$opp='эликсир';}
      $ret.="<div style=\"position:absolute; left:{$left}px; top:{$top}px; width:40px; height:25px; z-index:2\">
      <IMG width=40 height=25 src='".IMGBASE."/i/misc/icon_$inf_el[img]' onmouseout='ghideshow();' onmouseover='gfastshow(\"<B>".$rec['name']."</B> ($opp)<BR> еще ".lefttime($rec["time"])."\")';> </div>";
    }
  }
  return $ret;
}
...
function getslot($i) {
  ...
  foreach ($userslots as $k=>$v) if ($user[$v]==$i) return $v;
}
...
function usemagic($id,$target) {
    ...
    if (incommontower($user) && $row["bs"]!=$user["in_tower"]) return false;
    if ((($row['nsila']==0 || $user['sila'] >= $row['nsila']) &&
            ($row['nlovk']==0 || $user['lovk'] >= $row['nlovk']) &&
            ($row['ninta']==0 || $user['inta'] >= $row['ninta']) &&
            ($row['nvinos']==0 || $user['vinos'] >= $row['nvinos']) &&
            ($row['nintel']==0 || $user['intel'] >= $row['nintel']) &&
            ($row['nmudra']==0 || $user['mudra'] >= $row['nmudra']) &&
            ($row['gmp']==0 || $user['mana'] >= $row['gmp']) &&
            ($row['nlevel']==0 || $user['level'] >= $row['nlevel']) &&
            (((int)$user['align'] == (int)$row['nalign']) OR ($row['nalign'] == 0)) &&
            ($row['nnoj']==0 || $user['noj'] >= $row['nnoj']) &&
            ($row['ntopor']==0 && $user['topor'] >= $row['ntopor']) &&
            ($row['ndubina']==0 && $user['dubina'] >= $row['ndubina']) &&
            ($row['nmech']==0 && $user['mec'] >= $row['nmech']) &&
            ($row['type'] < 26 or $row['type'] == 49 or $row['type'] == 50 or $row['type'] == 54 or $row['type'] == 188) &&
            ($row['nfire']==0 || $user['mfire'] >= $row['nfire']) &&
            ($row['nwater']==0 || $user['mwater'] >= $row['nwater']) &&
            ($row['nair']>0 || $user['mair'] >= $row['nair']) &&
            ($row['nearth']==0 || $user['mearth'] >= $row['nearth']) &&
            ($row['nlight']==0 || $user['mlight'] >= $row['nlight']) &&
            ($row['ngray']==0 || $user['mgray'] >= $row['ngray']) &&
            ($row['ndark']==0 || $user['mdark'] >= $row['ndark']) &&
            ($row['needident'] == 0)
            ) || $row['magic'] == 48 || $row['magic'] == 50 || $row['magic'] == 139 || $row['magic'] == 203) {
...
function takeshopitem($item, $table="shop", $present="", $onlyonetrip="", $destiny=0, $fields=0, $uid=0) {
  global $user;
  if (!$uid) $uid=$user["id"];
...
function clearuser($id) {
  ...
  /*$r=mq("select * from effects where owner='$id' and (sila>0 or lovk>0 or inta>0 or vinos>0 or intel>0)");
  while ($rec=mysql_fetch_assoc($r)) {
    if ($rec["type"]>=11 && $rec["type"]<=14) mq("update users set sila=sila+'$rec[sila]', lovk=lovk+'$rec[lovk]', inta=inta+'$rec[inta]', vinos=vinos+'$rec[vinos]', intel=intel+'$rec[intel]' where id='$rec[owner]'");
    else mq("update users set sila=sila-'$rec[sila]', lovk=lovk-'$rec[lovk]', inta=inta-'$rec[inta]', vinos=vinos-'$rec[vinos]', intel=intel-'$rec[intel]' where id='$rec[owner]'");
    mq("delete from effects where id='$rec[id]'");
  }
  mq("delete from effects where owner='$id'");
  $r=mq("select * from obshagaeffects where owner='$id' and (sila>0 or lovk>0 or inta>0 or vinos>0 or intel>0)");
  while ($rec=mysql_fetch_assoc($r)) {
    if ($rec["type"]>=11 && $rec["type"]<=14) mq("update users set sila=sila+'$rec[sila]', lovk=lovk+'$rec[lovk]', inta=inta+'$rec[inta]', vinos=vinos+'$rec[vinos]', intel=intel+'$rec[intel]' where id='$rec[owner]'");
    mq("update users set sila=sila-'$rec[sila]', lovk=lovk-'$rec[lovk]', inta=inta-'$rec[inta]', vinos=vinos-'$rec[vinos]', intel=intel-'$rec[intel]' where id='$rec[owner]'");
    mq("delete from obshagaeffects where id='$rec[id]'");
  }
  mq("delete from obshagaeffects where owner='$id'");*/
  $e="";
  mq("delete from alleffects where owner='$id'");
  $e.=mysql_error();
  mq("insert into alleffects (select * from effects where owner='$id')");
  $e.=mysql_error();

  mq("delete from allobshagaeffects where owner='$id'");
  $e.=mysql_error();
  mq("insert into allobshagaeffects (select * from obshagaeffects where owner='$id')");
  $e.=mysql_error();

  mq("delete from allobshagastorage where pers='$id'");
  $e.=mysql_error();
  mq("insert into allobshagastorage (select * from obshagastorage where pers='$id')");
  $e.=mysql_error();
  ...
  mq("delete from allinventory where $oscond");
  $e.=mysql_error();
  mq("insert into allinventory (select * from inventory where $oscond)");
  $e.=mysql_error();


  $e.=mysql_error();
  mq("delete from allusers where id='$id'");
  $e.=mysql_error();
  mq("insert into allusers (select * from users where id='$id')");
  echo "insert into allusers (select * from users where id='$id')<br>";
  $z=mqfa1("select zver_id from users where id='$id'");
  $e.=mysql_error();
  if ($z) {
    mq("delete from allusers where id='$z'");
    $e.=mysql_error();
    mq("insert into allusers (select * from users where id='$z')");
    echo "insert into allusers (select * from users where id='$z')<br>";
    $e.=mysql_error();
  }
  mq("delete from alluserdata where id='$id'");
  $e.=mysql_error();
  mq("insert into alluserdata (select * from userdata where id='$id')");
  $e.=mysql_error();
  mq("delete from allonline where id='$id'");
  $e.=mysql_error();
  mq("insert into allonline (select * from online where id='$id')");
  $e.=mysql_error();
  mq("delete from allinventory where owner='$id'");
  $e.=mysql_error();
  mq("delete from allinventory where id in (select id from inventory where owner='$id')");
  $e.=mysql_error();
  mq("insert into allinventory (select * from inventory where owner='$id')");
  $e.=mysql_error();
  mq("delete from allputon where id_person='$id'");
  $e.=mysql_error();
  mq("insert into allputon (select * from puton where id_person='$id');");
  $e.=mysql_error();
  ...
  if (!$e) {
    ...
    if (!$o) {
      ...
      mq("delete from obshagaeffects where owner='$id'");
      echo mysql_error();
      //mq("delete from inventory where id in (select id_it as id from obshagastorage where pers='$id')");
...
function movebar() {
  ...
  <table cellspacing=0 cellpadding=0 style="position:absolute;left:0px;top:0px" width=100% border=0><tr><td>&nbsp;</td><td width="91">
...
function secs2hrs($s) {
  ...
  $d=floor($ret/24);
  $h=$ret%24;
  if ($d) return "$d д. $h ч. ".floor($s/60)." мин.";
  if ($h) return "$h ч. ".floor($s/60)." мин.";
  return floor($s/60)." мин. ".($s%60)." сек.";
}
...
function hassmallitems($item, $qty=1, $userid=0) {
  global $user;
  if (!$userid) $userid=$user["id"];
  $cnt=mqfa1("select sum(koll) from inventory where owner='$userid' and name='$item' and setsale=0");
  if ($cnt>=$qty) return 1;
  else return 0;
}
...
function haslinks($txt) {
  ...
  $txt=strtolower($txt);
  $txt=str_replace("virt-life.com/register.php","",$txt);
  $txt=str_replace("inqisitors-virt.ucoz.ua","",$txt);
  ...
  $txt=str_replace("virt-life.com","",$txt);
  $txt=str_replace("youtube.com","",$txt);
  $txt=str_replace("clansanctorum.ru","",$txt);
  $txt=str_replace("stalkers-virt.ucoz.org","",$txt);
  ...
  $txt=str_replace("angels0fwar.clan.su","",$txt);
  ...
  $txt=str_replace("virt-wov.clan.su","",$txt);
  ...
  //  fclose($f);
  //} 
  //if ($_SESSION["uid"]==7) return false;
  if (strpos(strtolower($txt),"devilwar")) return true;
  if (strpos(strtolower($txt),"legendworld")) return true;
  if (strpos(strtolower($txt),"worldofchaos")) return true; 
  if (strpos(strtolower($txt),"жарcаз")) return true;
  if (strpos(strtolower($txt),"казварс")) return true;
  if (strpos(strtolower($txt),"точcаkz")) return true;
  if (strpos(strtolower($txt),"com") && strpos($txt,"тoчcа")) return true;
...
hl?imover(lst[i].firstChild):imout(lst[i].firstChild);
  ...
  //if ($user['klan'] == 'Adminion' or $user['login']=='Модератор') $ret.=" <INPUT TYPE=button name=combats value=\"Одминские переходы\" onClick=\"location.href='main.php?setch=1';\" style=\"width: 150px;border: 1px double #9a9996;font-size: 12px;color: #dfdfdf;background-color:#504F4C;\"> ";
  ...
  if (in_array($user["room"], $clubrooms)) $ret.="<INPUT TYPE=button onClick=\"window.open('/forum.php', 'forum', 'location=yes,menubar=yes,status=yes,resizable=yes,toolbar=yes,scrollbars=yes,scrollbars=yes')\" class=\"btn\" style=\"border: 1px double #9a9996;font-size: 12px;color: #dfdfdf;background-color:#504F4C;\" value=\"Карта клуба\"> ";
  ...
  <INPUT TYPE=button onClick=\"document.location.href='/dialog.php?char=3';\" class=\"btn\" style=\"width:70px; border: 1px double #9a9996; font-size: 12px; color: #dfdfdf; background-color:#504F4C;\" value=\"Подсказка\"> ";
...
function canmove($spd=10) {
  global $user, $warning;
  $i=mqfa1("select id from fieldmembers where user='$user[id]'");
  if ($i) {
  ...
  $eff = mysql_fetch_array(mq("SELECT * FROM `effects` WHERE `owner` = '".$user['id']."' AND (`type` = 14 OR `type` = 13);"));
  if($eff) {
...
function updeffects($u=0) {
  ...
  while ($rec=mysql_fetch_assoc($r)) {
    if ($rec["type"]==28 || $rec["type"]==29) {
      ...
      $rec["addict"]=addictval($user["{$at}addict"]);
    }
    $data["effects"][]=$rec;
  }
...
function reportadms($msg) {
  addchp("<font color=\"Black\">private [Дядя Вася] private [Эллегия] private [Верховный] private [Модератор] $msg</font>", "Комментатор");
}
...
function updeffect($user, $effect, $time, $item, $hasaddict) {
  if ($hasaddict) {
  ...
  mq("UPDATE `effects` set `time` = '".(time()+$time)."' WHERE id='$effect[id]'");
}
...
function regenhp($us) {
  ...
  if ($us["hp"]<$us["maxhp"] && time()>$us["fullhptime"] && $us["in_tower"]<>62) {
    ...
    if ($delta>0) {
      ...
      mq("update users set hp=if(hp+$delta>maxhp,maxhp,hp+$delta) where id='$us[id]'");
...
function canregendead($user) {
  ...
  if ($user["in_tower"]==1 || $user["in_tower"]==2 || $user["in_tower"]==62) return false;
  return true;
}
...
function itemprice($rec) {
  ...
  if ($rec["name"]=="Страница книги алхимии") {
    ...
    $price=40*$rec["koll"];
  } elseif ($rec["name"]=="Самодельный эликсир") {
    $minprice=1;
    $maxprice=300;
    $price=1;
...
function cangive($sum) {
  ...
  if ($user["level"]<=5) $q=0.05;
  elseif ($user["level"]==6) $q=0.06;
  elseif ($user["level"]==7) $q=0.07;
  elseif ($user["level"]==8) $q=0.08;
  elseif ($user["level"]==9) $q=0.09;
  else $q=0.1;
...
if ($user["level"]==0 && $user["room"]!=1) gotoroom(1,0);
