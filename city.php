...
if ($_GET['portal'] && ($user['room']==73 or $user['room']==82)) {
...
die();
		...
		if ($_GET['got'] && $_GET['level3']) {
			...
			elseif ($user['level'] < 8) {
				print "<script>alert('Вход в комиссионный магазин только с 8 го уровня!')</script>";
			}
			else {
...
function Down(e) { if(e){ window.event = e; } top.CtrlPress = window.event.ctrlKey}
...
buildset(9000,"xram",150,257,"Храм",  '', 'Храм на реконструкции!');
buildset(35000,"vokzal",134,74,"Вокзал", '', 'Закрыто');
...
elseif ($user['room'] == 800000) { // Улиця Вязов
	if((int)date("H") > 6 && (int)date("H") < 22) {
		$fon = 'losttown001_1';
...
elseif ($user['room'] == 660) { // Парк развлечений
	if((int)date("H") > 6 && (int)date("H") < 22) {
		$fon = 'u2bg';
...
elseif ($user['room'] == 650) { // Большая торговая улица
	if((int)date("H") > 6 && (int)date("H") < 22) {
...
elseif ($user['room'] == 62) { // Портал (Большая торговая улица)
	if((int)date("H") > 6 && (int)date("H") < 22) {
...
echo "<table width=1><tr><td><div style=\"position:relative; cursor: pointer;\" id=\"ione\"><img src=\"i/city/",$fon,".jpg\" id=\"img_ione alt=\"\" border=\"0\"/ usemap=\"#moon_magic_portal2_map\">";

echo '<map name="moon_magic_portal2_map">
<area shape="rect" coords="80,90,165,200"  onclick="solo(\'20\',\'Бездна\', \'\')" id="mo_20" onmouseout="imout(this);" onmouseover="imover(this)" class="aFilter" alt="Вход в Бездну" title="Вход в Бездну">
<area shape="rect" coords="190,90,255,200" href="#" alt="Пещера тысячи проклятий" title="Пещера тысячи проклятий">
<area shape="rect" coords="280,90,345,200" onclick="solo(\'22\',\'Катакомбы\', \'\')" id="mo_22" onmouseout="imout(this);" onmouseover="imover(this)" class="aFilter" alt="Вход в катакомбы" title="Вход в катакомбы">
<area shape="rect" coords="370,85,430,200" href="#" alt="Пещера мглы" title="Пещера мглы">
</map>';
...
elseif ($user['room'] == 393) { // Проклятый Холм
