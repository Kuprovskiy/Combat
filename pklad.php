        ...
        if (!isset($_SESSION['pole_kopka_access'])) {
            if ($user['pole_kopka_last_visit']) { // если только зашел и раньше бывал
                if ((time() - $user['pole_kopka_last_visit']) < 3600) {
                    ...
                    header("Location: city.php?bps=1&level49=1");
                    die();
                }
            }
...
if($_POST['bonuskopka']){
 if($user['money'] >= 50){
mysql_query("update `users` set `pole_kopka_kol_bonus`=`pole_kopka_kol_bonus`+'1',`pole_kopka_kol_now`=`pole_kopka_kol_now`+'1',`pole_kopka_kol_all`=`pole_kopka_kol_all`+'1',`money`=`money`-'50' where `id`='".$user['id']."'");
...
if($_POST['x3']){
 if($user['money'] >= 85){
mysql_query("update `users` set `pole_kopka_update`='3',`pole_kopka_min`=`pole_kopka_min`*'3',`pole_kopka_max`=`pole_kopka_max`*'3',`money`=`money`-'85' where `id`='".$user['id']."'");
...
if($_POST['x5']){
 if($user['money'] >= 180){
mysql_query("update `users` set `pole_kopka_update`='5',`pole_kopka_min`=`pole_kopka_min`*'5',`pole_kopka_max`=`pole_kopka_max`*'5',`money`=`money`-'180' where `id`='".$user['id']."'");
...
if($_POST['x2']){
 if($user['money'] >= 70){
mysql_query("update `users` set `pole_kopka_update`='2',`pole_kopka_min`=`pole_kopka_min`*'2',`pole_kopka_max`=`pole_kopka_max`*'2',`money`=`money`-'70' where `id`='".$user['id']."'");
...
if($_POST['buy1']){
 ...
 if(!$lopata){
mysql_query("INSERT INTO `inventory` (`name`,`type`,`duration`,`maxdur`,`owner`,`img`,`present`,`isrep`) VALUES ('Копалка новичка','3','0','20','".$user['id']."','kopalka1.gif','Мусорщик','0')");
mysql_query("UPDATE `users` set `money`=`money`-'90',`pole_kopka_kol_now`='7',`pole_kopka_kol_all`='7',`pole_kopka_min`='5',`pole_kopka_max`='14',`pole_kopka_kol_bonus`='0',`pole_kopka_update`='0' where `id`='".$user['id']."'");
...
mysql_query("UPDATE `users` set `money`=`money`-'120',`pole_kopka_kol_now`='9',`pole_kopka_kol_all`='9',`pole_kopka_min`='8',`pole_kopka_max`='19',`pole_kopka_kol_bonus`='0',`pole_kopka_update`='0' where `id`='".$user['id']."'");
...
<? if ($user["room"] ==58) echo "<input type=hidden name=bps value=\"1\"><input type=hidden name=level49 value=\"1\">"; ?>
...
?> 
<? if ($user["align"] ==2.5) { ?>
<form method="GET" name="faskprize">
<INPUT TYPE="button" onClick="location.href = 'pklad.php?nulled=1';" value="Обнулить Поле">
</form>
<?}?>
...
<input type=submit class=input name=bonuskopka value='Купить +1 копку'> <small>Позволяет копнуть на 1 раз больше. Цена: <b>50</b> Кр</small><hr>";
...
<input type=submit class=input name=x2 value='Улучшить глубину х2'> <small>Увеличивает глубину копания копалкой в 2 раза! Цена: <b>70</b> Кр</small><br>
<input type=submit class=input name=x3 value='Улучшить глубину х3'> <small>Увеличивает глубину копания копалкой в 3 раза! Цена: <b>85</b> Кр</small><br>
<input type=submit class=input name=x5 value='Улучшить глубину х5'> <small>Увеличивает глубину копания копалкой в 5 раза! Цена: <b>180</b> Кр</small>";
...
echo"<table width=400 valign=top><tr valign=top><td width=200 align=left><center><small>Копалка новичка<br><img src=i/sh/kopalka1.gif></center>Кол-во копаний: <b>7</b><br>Глубина копки: <b>-5</b> .. <b>-14</b><br>Стоимость: <b>75</b> Кр</small><center><input type=submit class=input name=buy1 value='Купить'></center></td><td width=200 align=left><center><small>Копалка мастера<br><img src=i/sh/kopalka2.gif></center>Кол-во копаний: <b>9</b><br>Глубина копки: <b>-8</b> .. <b>-19</b><br>Стоимость: <b>100</b> Кр</small><center><input type=submit class=input name=buy2 value='Купить'></td></tr></table>";
}
