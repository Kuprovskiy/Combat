<?
  ...
  function pickupitem($item, $foronetrip, $notmore1, $incave=0, $podzem=1) {
    global $user;
    if ($notmore1) {
    ...
    $taken=takeshopitem($item, "shop", "", $foronetrip, 0, $flds);
    return "Вы получили <b>$taken[name]</b>.";
  }

  function itemtofloor($item, $foronetrip, $incave=0, $podzem=1) {
    global $user, $x, $y, $floor;
    $rec=mqfa("select name, img from shop where id='$item'");
    mq("insert into caveitems set leader='$user[caveleader]', name='$rec[name]', img='$rec[img]', x='".($x*2)."', y='".($y*2)."', floor='$floor', item='$item', foronetrip='$foronetrip', incave='$incave', podzem='$podzem'");
  ...
  if (@$_GET["useitem"] || @$_GET["usewallitem"]) mq("lock tables userdata write, obshagaeffects write, effects write, cavebots write, quests write, battle write, users write, droplog write, caveparties write, caves write, smallitems write, shop write, inventory write, bots write, caveitems write");
  loadmap();
      ...
      if ($it["small"]) {
        ...
        if (!placeinbackpack(1)) {
          ...
          if ($it["item"]==2347) {
            ...
            $taken=takeshopitem($it["item"], "shop", "", $it["foronetrip"], 0, array("podzem"=>$it["podzem"], "incave"=>$it["incave"]), 0, 1, "Нашёл в пещере");
            if (@$taken["error"]) {
              $report=$taken["error"];
            } else {
              mq("delete from caveitems where leader='$user[caveleader]' and x='".($x*2)."' and y='".($y*2)."' and floor='$floor' and id='$_GET[takeitem]'");
...
<?
  ...
  function drawmap($map1, $players, $x, $y, $direction) {
    ...
    //$x=$players[0]["x"];
    //$y=$players[0]["y"];
    echo $startx=max($x*2-8,0);
    echo $starty=max($y*2-8,0);
...
if ($user["id"]==7) { 
    ...
    echo '</pre>'; 
    echo '<pre style="float: left">';
    //print_r(unserialize(mqfa1('SELECT map FROM cavemaps WHERE floor = 1 AND room = ' . ($user['room'] - 1))));
    print_r($party);
    echo '</pre>'; 
    echo '<pre style="float: left">';
    print_r($imgdata);
