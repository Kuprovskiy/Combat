...
?>
<HTML>
<HEAD><TITLE>Форум Oldbk2.com</TITLE>
...
<TABLE height="100%" cellSpacing=0 cellPadding=0 width="100%" border=0>
  ...
  <TR>
    ...
    <TD vAlign=top width="70%">
      ...
      <TABLE cellSpacing=0 cellPadding=0 width="100%" border=0>
        <TBODY>
          <TD width="50%" bgColor=#f2e5b1 height=100></TD>
          <TD width=269 height=100 align="right"><IMG height=100 src="i/forumru.jpg" width=269 border=0></TD>
      ...
      <TABLE cellSpacing=0 cellPadding=0 width="100%" bgColor=#f2e5b1
        ...
        <TR>
          ...
          <TD vAlign=top width="15%" rowSpan=2><BR><BR>
            ...
            <TABLE height="90%" cellSpacing=0 cellPadding=0 border=0>
              <TBODY>
              <TR>
                <TD vAlign=top ground=""><IMG height=292 src="i/paper1.jpg" width=39
        ...
        if($_POST['add'] && $user['id'] && $user['level']>7) {
        ...
        if($_POST['add2'] && $user['id'] && $user['level']>7) {
        //if((time()-30)>$_COOKIE['time']) {
            ...
            if ($user['level'] < 8 or $user['align'] == 4 or $minmax['close'] == 1){echo "<font color=red><b>Не выйдет...</b></font>";}else{
            if ($user['invis']=='1' && $user["id"]!=7) {
...
//          else
            ...
            <?
                ...
                if ($closed==1) {
                    ...
                    if ( $_SESSION['uid'] && !$effect['time'] && $user['align'] != 4 && $user['level'] > 7) {
          ?>
            <A name=answer></A>
            ...
            <P> <BR><BR></P></TD></TR><?
                    ...
                    elseif ($user['level'] < 8) {
                        echo "<br><br><center>Персонажам до 8-го уровня запрещено писать на форуме!</center><br><br>";
                    }
                    else {
                        echo "<br><br>";
                    }
            ...
            $effect = mysql_fetch_array(mysql_query("SELECT `time` FROM `effects` WHERE (`owner` = '{$user['id']}' or owner=$user[id]+"._BOTSEPARATOR_.") and (`name` = 'Заклятие форумного молчания' or type=3) LIMIT 1;"));
            if ( $_SESSION['uid'] && !$effect['time'] && $user['align'] != 4 && $user['level'] > 7 && (($_GET["conf"]!=10 && $_GET["conf"]!=3) || $user["id"]==7)) {
