<?php
    ...
    if ($_GET['seds']) {
        ...
        $dress = mysql_fetch_array(mysql_query("SELECT * FROM `inventory` WHERE `dressed`= 0 AND `id` = '{$_GET['seds']}' AND `owner` = '{$_SESSION['uid']}'  AND `podzem`=0 AND `artefact` = 1 LIMIT 1;"));
...
if ($_REQUEST['sales']) {
    $data = mysql_query("SELECT * FROM `inventory` WHERE `owner` = '{$_SESSION['uid']}' AND `honor` = 0 AND `artefact` = 1 AND `dressed` = 0  AND `setsale`=0  AND `podzem`=0 AND `gift`=0 AND `honor`=0 AND type<>199 and `ecost`>0 ORDER by `update` DESC; ");
    while($row = mysql_fetch_array($data)) {
        $row['count'] = 1;
        if ($i==0) { $i = 1; $color = '#C7C7C7';} else { $i = 0; $color = '#D5D5D5'; }
        echo "<TR bgcolor={$color}><TD align=center style='width:150px'><IMG SRC=\"".IMGBASE."/i/sh/{$row['img']}\" BORDER=0>";
        $price=$row['ecost']*SELLCOEF;
        ?>
            <BR><A HREF="berezka.php?seds=<?=$row['id']?>&sid=&sales=1">продать за <?=floor($row['ecost']*SELLCOEF*(($row["maxdur"]-$row["duration"])/$row["maxdur"])*10)/10?> екр</A>
       </TD>
        <?php
        echo "<TD valign=top>";
        showitem ($row);
...
{
    $data = mysql_query("SELECT * FROM `berezka` WHERE `count` > 0 AND `razdel` = '{$_GET['otdel']}' ORDER by `nlevel` ASC, `ecost` ASC");
    while($row = mysql_fetch_array($data)) {
        if ($i==0) { $i = 1; $color = '#C7C7C7';} else { $i = 0; $color = '#D5D5D5'; }
        echo "<TR bgcolor={$color}><TD align=center style='width:150px'><IMG SRC=\"".IMGBASE."/i/sh/{$row['img']}\" BORDER=0>";
        ?>
