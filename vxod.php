...
if($el = mysql_fetch_array ($select)) {
  ...
  $left_sec=$left_time-$left_min*100;
...
<?
  ...
  if (in_array($user["room"], array(82, 73)))
  {
	  foreach ($routes[$user["room"]] as $k=>$v) $links[$rooms[$v]]="city.php?portal=1&level$v=1";
  }
  else
  {
	  foreach ($routes[$user["room"]] as $k=>$v) $links[$rooms[$v]]="city.php?hill=1&park=1&altar=1&sklon=1&bps=1&level$v=1";
  }
  echo moveline($links);
?>
<BR>
...
<?
  ...
  if ($user["room"]==64) {
echo "Пещера Загадок - это место, где проводятся праздничные викторины. Принять участие могут все желающие.
Для того, чтобы принять участие, необходимо после начала викторины нажать кнопку создать группу и потом начать.
После этого персонаж попадает в пещеру, где для получения приза необходимо добраться до выхода.
Поединков в пещере нет, нужно только отвечать на вопросы. Попав в пещеру, вы окажетесь в корридоре,
по которому надо идти вперёд (коридор петляет, но ответвлений нет).
Через определённые промежутки пути расположены знаки. Нажав на этот знак появится вопрос с тремя
вариантами ответа. После этого надо идти в комнату, которая расположена слева от знака с вопросом,
в ней будут 3 знака с буквами ответа. Надо нажать на букву, которая соответствует выбранному ответу.
Если ответ верный, вы попадёте в корридор, в конце которого будет следующий вопрос. Если неверный,
то в начало этого же корридора, пройдя который можно будет выбрать другой ответ на этот вопрос.
Первые 10 ответивших на все 12 вопросов получают 10 сущностей ресурса, 10 эссенций крови или кровавых
рубинов и награду победителя викторины (без срока годности, отображается в информации в разделе награды и трофеи),
также все участники, которые ответят правильно на все вопросы, получают приз за участие: 1 сущность ресурса,
1 эссенцию крови или кровавый рубин. и памятный подарок (срок годности 30 дней, в информации о персонаже
отображается вместе с обычными подарками). Кровавые рубины получают персонажи, которые пожертвовали хотя
бы один рубин на алтаре крови, остальные получают эссенции крови. Начать поход можно не позже чем через час после
начала викторины. Время на прохождение пещеры неограничено. Конечно, чем позже вы начнёте, тем меньше
шансов получить призовое место, но приз за участие получат все, кто ответит на все вопросы.";
}
?><br><br>
</BODY>
</HTML>
