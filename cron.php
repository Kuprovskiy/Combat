...
if ($users_count>=200 && !mqfa1("SELECT COUNT(*) FROM visitors_counter WHERE date = CURDATE()")) {
    ...
    sysmsg("Сейчас в клубе 200 чел. В связи с этим, всем кто присутствует в игре зачислены кредиты."); 
}
*/
...
if ($cLottery) {
    ...
    $cLottery = mysql_result(mysql_query('SELECT COUNT(*) FROM lottery WHERE end = 0 AND date < NOW()'), 0, 0);
    if (!$cLottery) {
        sysmsg("Уважаемые игроки! Закончился очередной тираж Лотереи. Чтобы ознакомиться с результатами и получить выигрыш, посетите данную локацию в Old City.");    
    }
}
  ...
  while ($rec=mysql_fetch_assoc($r)) {
    ...
    if (!$tr) {
      ...
      if($dd[0] < 2 && $turnirstart[0] <= time()) {
		...
		sysmsg("Турнир Башня Смерти начнется через 6 часов");
      }
  ...
  if (($ldfield["team1"]=="-" || $ldfield["team2"]=="-") && !$ldfield["pts1"] && !$ldfield["pts2"]) {
    ...
    if ($winner) {
      ...
      foreach ($winners as $k=>$v) {
        if (!$v) continue;
        mq("INSERT INTO `effects` set owner=$v, name='Победитель общего побоища', time=".(60*60*24+time()).", type=186, mfdhit=100, mfdmag=100, mf='mfhitp/mfmagp', mfval='20/20'");
...
//echo"Последнее значение:".$info_pole['value'];
$ff_time = time();
